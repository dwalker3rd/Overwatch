...
#region STATUS
    ...
    function global:Get-AlteryxServerStatus {
        ...
        if ($controllerService) {
            ...
            $controllerServiceStatus += @{IsOK = $controllerService ? $PlatformServiceUpState -in $controllerServiceStatus.Status : $true}
        ...
        if (!$platformTopology.Components.Controller.EmbeddedMongoDBEnabled) {
            ...
            if ($databaseService) {
                ...
                $databaseServiceStatus += @{IsOK = $databaseService ? $PlatformServiceUpState -in $databaseServiceStatus.Status : $true}
        ...
        if ($galleryService) {
            ...
            $galleryServiceStatus += @{IsOK = $galleryService ? $PlatformServiceUpState -in $galleryServiceStatus.Status : $true}
        ...
        if ($workerService) {
            ...
            $workerServiceStatus += @{IsOK = $workerService ? $PlatformServiceUpState -in $workerServiceStatus.Status : $true}
...
#region TOPOLOGY
    ...
    function global:Initialize-PlatformTopology {
        ...
        foreach ($node in $Nodes) {
            ...
            $platformTopology.Alias.($node.replace("ayx-","").replace("-0","")) = $node
            # $platformTopology.Alias.$node = $node.replace("ayx-","").replace("-0","")
        }
        ...
        foreach ($component in $Components) {
            ...
            $platformTopology.Components.$component.ScaleSet = $ScaleSet.$component
            $platformTopology.Components.$component.Standby = @()
            foreach ($node in $ScaleSet.$component) {
                if ($node -notin $Nodes) {
                    $platformTopology.Components.$component.Standby += $node
                }
            }
        }
        ...
        # $platformTopology.Components.Controller.Passive = "ayx_control_02"
