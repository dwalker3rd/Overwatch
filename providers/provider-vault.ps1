...
function global:Move-ToVault {

    param(
        [Parameter(Mandatory=$false)]
        [ValidateSet("tableau-acme-org","tableautest-acme-org","alteryx-acme-org")]
        [string]$PlatformInstance = $global:Platform.Instance
    )

    $currentPlatformInstance = $Platform.Instance
    $global:Platform.Instance = $PlatformInstance

    $credentialFiles = get-childitem "f:\overwatch\data\$platforminstance\*.secure" -recurse -Force
    foreach ($credentialFile in $credentialFiles) {
        $credentialName = $credentialFile.Name.Replace(".secure","")
        Write-Host+ -notrace -NoTimestamp "Migrating $credentialFile to Overwatch vault"
        $creds = Get-Credentials $credentialName
        if ($creds) {
            Set-Credentials $credentialName -Credentials $creds
            Get-FromSecretVault $credentialName
            Rename-Item -path $credentialFile -NewName "$credentialFile.migratedToVault"
        }
    }

    $vaultContent = read-cache vault
    $vaultKeys = read-cache keyvault
    foreach ($name in $vaultContent.keys) {
        $vaultSecret = Get-FromSecretVault $name
        if (!$vaultSecret) {
            Write-Host+ -notrace -NoTimestamp "Migrating $name from cache-based vault to Overwatch vault" 
            $vaultSecret = $vaultContent.$name
            $vaultKey = $vaultKeys.$name
            $username = $vaultSecret.keys[0]
            $password =  $vaultSecret.$($username) | ConvertTo-SecureString -Key $vaultKey
            $creds = New-Object System.Management.Automation.PSCredential($userName, $password)
            Set-Credentials $name -Credentials $creds
            Get-FromSecretVault $name
        }
    }

    $cacheKeyVault = "f:\overwatch\data\$platforminstance\keyvault.cache"
    $cacheSecretVault = "f:\overwatch\data\$platforminstance\vault.cache"
    if (Test-Path $cacheKeyVault) {Rename-Item -path $cacheKeyVault -NewName "$cacheKeyVault.migratedToVault"}
    if (Test-Path $cacheSecretVault) {Rename-Item -path $cacheSecretVault -NewName "$cacheSecretVault.migratedToVault"}

    $global:Platform.Instance = $currentPlatformInstance
