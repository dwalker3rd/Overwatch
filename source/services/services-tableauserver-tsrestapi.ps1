...
#region SYNC
    ...
    function global:Sync-TSGroups {
        ...
        foreach ($contentUrl in $global:AzureADSync.Sites.ContentUrl) {
            ...
            $tsGroups = Get-TSGroups | Where-Object {$_.name -in $azureADGroups.displayName} | Select-Object -Property *, @{Name="site";Expression={$null}} | Sort-Object -Property name
            ...
            foreach ($tsGroup in $tsGroups) {
                ...
                foreach ($azureADUser in $azureADUsersToAddToSite) {
                    ...
                    $newUsers += $newUser
                    # Write-Log -Context "AzureADSync" -Action "AddUser" -Target "$($tsSite.contentUrl)\$($newUser.name)" -Force 
                    # $tsUsersToAddToGroup += $newUser
                    # $tsUsers += $newUser
                }
...
function global:Get-TSUsersDistributionLists {
    ...
    foreach ($site in $sites) {
        ...
        if ($ExcludePATHUsers) {
            $siteUsers = $siteUsers | Where-Object {$_.name -notlike "*@acme.org"}
        }
