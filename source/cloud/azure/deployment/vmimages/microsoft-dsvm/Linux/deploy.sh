...
main_storage_account_key=$(terraform output -raw main_storage_account_key)
secondary_storage_account_name=$(terraform output -raw secondary_storage_account_name)
secondary_storage_account_key=$(terraform output -raw secondary_storage_account_key)
RESOURCE_GROUP_NAME=$(terraform output -raw resource_group_name)
compute_name=$(terraform output -raw compute_name)
vm_fqdn_label=$(terraform output -raw vm_fqdn)
key_vault_name=$(terraform output -raw key_vault_name)
vm_fqdn="$vm_fqdn_label.$RESOURCE_LOCATION.cloudapp.azure.com"
PD_CLIENT_ID=$(terraform output -raw client_id)
PD_CLIENT_SECRET=$(terraform output -raw client_secret)

printed_name=$(az rest --method GET --uri "https://graph.microsoft.com/v1.0/users/$USER_ID" --query displayName | tr -d '"')
printed_name=${printed_name:0:32}
...
printed_name=${printed_name//.}
printed_name=${printed_name//,}
initial_username=$printed_name

INITIAL_USER_EMAIL=$(az rest --method GET --uri "https://graph.microsoft.com/v1.0/users/$USER_ID" --query mail)

# VM Extension
# Zip the configuration files and the post deployment script
echo
zip -r ./config.zip ./configuration-files/etc ./configuration-files/lib ./configuration-files/usr
echo 'Done zipping configuration files...'

# Push the configuration files to storage
echo 'Uploading configuration files (config.zip)'
az storage blob upload --container-name deployment --file config.zip --name config.zip --account-key $secondary_storage_account_key --account-name $secondary_storage_account_name
echo 'Done uploading all files...'
sleep 10

echo 'Getting sas key for config.zip'
tf_config_zip_sas_key=''
if [[ -n $AZUREPS_HOST_ENVIRONMENT ]]; then  
    echo "Running in Azure Cloud Shell" 
    expiry_date=$(date -u -d "+1 day" +"%Y-%m-%dT%H:%M:%SZ")
    tf_config_zip_sas_key=$(az storage blob generate-sas --account-name $secondary_storage_account_name --account-key $secondary_storage_account_key --container-name deployment --name config.zip --permissions r --expiry "$expiry_date" --output tsv)
else  
...
download_config_file_full_url="https://$secondary_storage_account_name.blob.core.windows.net/deployment/config.zip?$tf_config_zip_sas_key"
echo "Download config.zip url: $download_config_file_full_url"

# Update post deployment script variables:
PD_SKIP_CERT='FALSE'
# Create an output file that has all the variables and steps to do the post deployment tasks:
echo "passed_values=('$download_config_file_full_url' $main_storage_account_name $main_storage_account_key data $vm_fqdn $INITIAL_USER_EMAIL $initial_username $PD_CLIENT_ID $PD_CLIENT_SECRET $TENANT_ID $PD_SKIP_CERT)" >> tf-post-deploy-output.sh
echo "$(cat post-deploy-template.sh)" >> tf-post-deploy-output.sh

echo 'Uploading tf-post-deploy-output.sh'
az storage blob upload --container-name deployment --file tf-post-deploy-output.sh --name tf-post-deploy-output.sh --account-key $secondary_storage_account_key --account-name $secondary_storage_account_name
echo 'Done uploading tf-post-deploy-output.sh...'

tf_pd_script_sas_key=''
if [[ -n $AZUREPS_HOST_ENVIRONMENT ]]; then  
    echo "Running in Azure Cloud Shell" 
    expiry_date=$(date -u -d "+1 day" +"%Y-%m-%dT%H:%M:%SZ")
    tf_pd_script_sas_key=$(az storage blob generate-sas --account-name $secondary_storage_account_name --account-key $secondary_storage_account_key --container-name deployment --name tf-post-deploy-output.sh --permissions r --expiry "$expiry_date" --output tsv)
else  
    echo "Not running in Azure Cloud Shell"  
    expiry_date=$(date -u -d "+1 day" +"%Y-%m-%dT%H:%M:%SZ")  
    tf_pd_script_sas_key=$(az storage blob generate-sas --account-name $secondary_storage_account_name --account-key $secondary_storage_account_key --container-name deployment --name tf-post-deploy-output.sh --permissions r --expiry "$expiry_date" --output tsv)
...
download_pd_script_full_url="https://$secondary_storage_account_name.blob.core.windows.net/deployment/tf-post-deploy-output.sh?$tf_pd_script_sas_key"
echo "Download file url: $download_pd_script_full_url"

# Set extension in vm
echo "Setting extension in VM. Excuting post deploy script..."
az vm extension set \
--resource-group $RESOURCE_GROUP_NAME \
--vm-name $compute_name \
--name customScript \
--publisher Microsoft.Azure.Extensions \
--protected-settings '{"fileUris": ["'$download_pd_script_full_url'"],"commandToExecute": "sudo bash tf-post-deploy-output.sh"}'

sleep 20
# Reboot the vm
az vm restart -g $RESOURCE_GROUP_NAME -n $compute_name


# Delete the configuration files used for deployment
az storage blob delete --container-name deployment --name config.zip --account-key $secondary_storage_account_key --account-name $secondary_storage_account_name
# Delete the deployment container 
az storage container delete --name deployment --account-key $secondary_storage_account_key --account-name $secondary_storage_account_name

# set the storate account to be private network access only
az storage account update --name $secondary_storage_account_name --resource-group $RESOURCE_GROUP_NAME --allow-blob-public-access false

sleep 100

nsg_name=$(terraform output -raw nsg_name)
# Remove rule for port 80, only need it to generate the certificate.
az network nsg rule delete -g $RESOURCE_GROUP_NAME --nsg-name "$nsg_name" -n "Port_80"
...
echo ''
echo 'This is the generated private key, this is saved to Key Vault as a secret.'
echo $ssh_private_key
echo 'Go to this url to connect to the vm thru web based JupyterLab:'
echo "https://${vm_fqdn}:8000"
echo ''
echo '====================================================================================='