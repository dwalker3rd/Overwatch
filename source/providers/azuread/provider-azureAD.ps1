...
function global:Update-AzureADUserNames{
    ...
    $params = @{}
    if (![string]::IsNullOrEmpty($GivenName)) { $params += @{ givenName = $givenName } }
    if (![string]::IsNullOrEmpty($surName)) { $params += @{ surName = $surName } }
    if (![string]::IsNullOrEmpty($displayName)) { $params += @{ displayName = $displayName } }
